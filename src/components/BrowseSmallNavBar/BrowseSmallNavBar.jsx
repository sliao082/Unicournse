...
const BrowseSmallNavBar = () => {
    ...
    if (!user) {
        return (
            <div className="small-nav-bar" style={{ justifyContent: "flex-end" }}>
    ...
    const dynamicUserId = userInfo && Array.isArray(userInfo.info.userid) && userInfo.info.userid.length > 0 ? userInfo.info.userid[0] : "";

    return (
        <div className="small-nav-bar">
            <Link to="/" className="nav-bar-logo"></Link>
            <div className="nav-bar-user">
                <Link className="nav-bar-user-icon" to={`/user/${dynamicUserId}/settings`}>
                ...
                <Link className="nav-bar-user-name" to={`/user/${dynamicUserId}/profile`}>
...
export default BrowseSmallNavBar