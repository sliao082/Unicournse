        ...
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            if (!user) {
                ...
                if (docSnap.exists()) {
                    ...
                    if (data?.info?.userid && Array.isArray(data.info.userid)) {
                        const correctId = data.info.userid[0];
                        if (correctId !== id) {
                            const prefix = `/user/${id}`;
                            const suffix = location.substring(prefix.length);
                            navigate(`/user/${correctId}${suffix}`);
                        }
                    }
                }
            }
    ...
    return (
        <div className="user-container">
            <div className="user-side-bar">
                <Link className="side-bar-logo" to="../"></Link>
                <div className="side-bar-list">
                    ...
                    <button className='side-bar-item unselected' onClick={handleLogout}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M5 22C4.44772 22 4 21.5523 4 21V3C4 2.44772 4.44772 2 5 2H19C19.5523 2 20 2.44772 20 3V6H18V4H6V20H18V18H20V21C20 21.5523 19.5523 22 19 22H5ZM18 16V13H11V11H18V8L23 12L18 16Z"></path></svg>
                        <p>Log Out</p>
                    </button>
                </div>
            </div>
