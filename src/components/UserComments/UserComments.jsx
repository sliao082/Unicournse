import './style.css';

const UserComments = () => {
    // Sample data
    const comments = [
        {
            id: 1,
            course: 'CS 124',
            title: 'Introduction to Computer Science',
            comment: 'Great course for understanding fundamental programming concepts!',
            date: '2024-03-15'
        },
        {
            id: 2,
            course: 'STAT 431',
            title: 'Bayesian Statistics',
            comment: 'Challenging but rewarding. Professor was very helpful.',
            date: '2024-02-28'
        },
        {
            id: 3,
            course: 'STAT 431',
            title: 'Bayesian Statistics',
            comment: 'Challenging but rewarding. Professor was very helpful.',
            date: '2024-02-28'
        },
        {
            id: 4,
            course: 'STAT 431',
            title: 'Bayesian Statistics',
            comment: 'Challenging but rewarding. Professor was very helpful.',
            date: '2024-02-28'
        }
    ];

    return (
        <div className="user-main-block">
            <div className="user-main-header">
                <h2>My Comments</h2>
            </div>
            <div className="comments-container">
                {comments.map((comment) => (
                    <div key={comment.id} className="comment-card">
                        <div className="comment-content">
                            <div className="course-header">
                                <h3 className="course-code">{comment.course}</h3>
                                <span className="course-title">{comment.title}</span>
                            </div>
                            <p className="comment-text">{comment.comment}</p>
                            <div className="comment-date">
                                Posted on {new Date(comment.date).toLocaleDateString()}
                            </div>
                        </div>
                        <div className="comment-actions">
                            <button className="icon-button edit-button">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="var(--button-color)">
                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                </svg>
                            </button>
                            <button className="icon-button delete-button">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="var(--button-color)">
                                    <path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" />
                                </svg>
                            </button>
                        </div>
                    </div>
                ))}
            </div>
        </div>
    );
};

export default UserComments